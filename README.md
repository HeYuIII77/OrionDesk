...
| **CMD 启动器** | 大图标一键启动 CMD 并自动执行多条命令（可自定义图标/名称/命令/起始目录） |
| **桌面球** | 悬浮小球，双击切换组件置顶/桌面层（置顶时组件浮在所有应用上面，点击其他应用自动恢复） |
...
**CMD 启动器**
- 点击大图标启动 CMD 并自动执行命令
- 支持多条命令（每行一条，用回车分隔，依次执行）
- 右键 → 设置 → 配置显示名称、图标（emoji）、启动命令、起始目录
- 默认执行 `claude`，起始目录 `C:\WINDOWS\system32`

**桌面球**
- 托盘右键 → 桌面球 创建悬浮小球
- 单击拖动移动位置，靠近屏幕边缘自动吸附（留40%在外面）
- 双击切换：组件置顶（浮在所有应用上面）/ 恢复桌面层
- 置顶模式下组件背景变为不透明，点击其他应用自动恢复
- 位置自动保存，下次启动自动恢复
